...
async function main(): Promise<void> {
  ...
  const observations = [
    ...
    NINE_STAR_KI_FICTIONAL_FIXTURE,
  ] as const;

  // Each method gets an independent API call and retains its own raw evidence.
  // Running all three together keeps model/configuration conditions aligned for comparison.
  ...
  const runId = `daily-v0.1-three-method-fictional-${executionTimestamp.replace(/[:.]/g, "-")}`;
  const run: DailyRun = {
    schemaVersion: "0.1",
    runId,
    executionTimestamp,
